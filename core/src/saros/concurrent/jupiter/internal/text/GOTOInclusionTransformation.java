...
public class GOTOInclusionTransformation implements InclusionTransformation {
  ...
  private Operation transform(DeleteOperation delA, InsertOperation insB) {
    ...
    if (startB.compareTo(endA) >= 0) {
      ...
      if (lineDeltaBeforeSplit == 0) {
        ...
        splitOffset =
            StringUtils.lastOrdinalIndexOf(text, lineSeparator, lineDeltaBeforeSplit)
                + lineSeparator.length()
                + offsetDeltaBeforeSplit;
      }
    ...
    if (startB.compareTo(endA) >= 0) {
      ...
      if (startB.compareTo(startA) <= 0 && endA.compareTo(endB) <= 0) {
        ...
        if (endDeletion.getLineDelta() > 0) {
          adjustedOffsetDelta = endDeletion.getLineDelta();
  ...
  private DeleteOperation dropLeadingOverlap(DeleteOperation delA, DeleteOperation delB) {
    ...
    if (droppedLines == 0) {
      int droppedOffset = inLineOffsetEndB - startA.getInLineOffset();

      adjustedText = text.substring(droppedOffset);

      if (lineDeltaA == 0) {
        adjustedOffsetDelta = offsetDeltaA - droppedOffset;
      } else {
      ...
      int droppedOffset = inLineOffsetEndB;
      ...
      int newStartOffset = splitLineOffset + lineSeparator.length() + droppedOffset;
  ...
  private DeleteOperation dropTrailingOverlap(DeleteOperation delA, DeleteOperation delB) {
    ...
    if (droppedLines == 0) {
      int droppedOffset = endA.getInLineOffset() - inLineOffsetStartB;

      adjustedText = text.substring(0, text.length() - droppedOffset);
      ...
      //noinspection UnnecessaryLocalVariable
      int droppedOffset = inLineOffsetStartB;

      // TODO remove guess once we have implemented content normalization
      String lineSeparator = TextPositionUtils.guessLineSeparator(text);

      int splitLineNumber = lineDeltaA - droppedLines;

      int splitLineOffset = StringUtils.ordinalIndexOf(text, lineSeparator, splitLineNumber);
      if (splitLineOffset == -1) {
        throw new IllegalStateException(
            "Could not find line separator " + splitLineNumber + " in text");
      }

      int newStartOffset = splitLineOffset + lineSeparator.length() + droppedOffset;

      adjustedText = text.substring(0, newStartOffset);
