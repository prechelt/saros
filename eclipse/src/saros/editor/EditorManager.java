...
public class EditorManager implements IEditorManager {
  ...
  void textAboutToBeChanged(int offset, String text, int replaceLength, IDocument document) {
    ...
    TextEditActivity textEdit =
        new TextEditActivity(session.getLocalUser(), startPosition, text, replacedText, path);
  ...
  private void execTextEdit(TextEditActivity textEdit) {
    ...
    try {
      ...
      replaceText(path, offset, replacedText, text, user, doc, annotationModel);
   ...
   * @param documentAnnotationModel the annotation model obtained from the document provider for the
   *     file
   */
  private void replaceText(
      ...
      User source,
      IDocument doc,
      IAnnotationModel documentAnnotationModel) {
    ...
    contributionAnnotationManager.insertAnnotation(documentAnnotationModel, offset, length, source);
  }
